...
let dopamineRewardPulse=0.0, acousticPulse=0.0, lastRewardScore=0;
...
function display(s){
  shown=s;setFrame(s.frame);$('stage-feed').textContent=recording?'REPLAY / FLY ROOM':'LIVE / FLY ROOM';
  ...
  if(s.driver==='neural'&&s.neural.readout_kind==='calibrated')$('brain-note').textContent='BCI calibrado sobre spikes de la lámina. Proyección y contraste experimentales.';
  const l=s.learning;
  if(l&&l.enabled){
  ...
  $('decoder-method').textContent=calibrated?'Un lector externo calibrado convierte actividad reciente de poblaciones de lámina en cuatro pulsaciones. Durante el juego solo recibe spikes; los pesos del conectoma permanecen fijos.':'Cuatro grupos de lectura generan pulsaciones mediante un mapeo fijo, sin acceso a notas futuras ni recompensas.';
  ...
  const event=s.game.events.at(-1), key=event?`${s.game.time}:${event.kind}:${event.lane}`:'';
  if(event&&key!==lastEvent){
    ...
    if(event.kind==='perfect'||event.kind==='good'){
      dopamineRewardPulse=1.0;
    }
  }
  if((s.learning?.last_reward&&s.learning.last_reward>0)||(s.game?.score&&s.game.score>lastRewardScore)){
    dopamineRewardPulse=1.0;
  }
  lastRewardScore=s.game?.score||0;
...
function syncSongAudio(s) {
  ...
  if (s.mode === 'running' && (!recording || playback)) {
    if (Math.abs(songAudio.currentTime - s.game.time) > 0.15) {
      songAudio.currentTime = Math.max(0, s.game.time);
    }
    if (songAudio.paused) {
...
$('start').onclick=()=>{
  if (sound && live?.mode !== 'running') {
    ...
    songAudio.currentTime = Math.max(0, live?.game?.time || 0);
    songAudio.play().catch(e => console.warn('Audio play on start gesture:', e));
  }
...
function press(lane){if(live?.mode==='running'&&live.driver==='manual'&&!recording)control({type:'keys',lanes:[lane]});}
document.querySelectorAll('[data-lane]').forEach(button=>button.addEventListener('pointerdown',e=>{e.preventDefault();press(Number(button.dataset.lane));}));
document.querySelectorAll('[data-lane]').forEach(button=>button.addEventListener('click',e=>{if(e.detail===0)press(Number(button.dataset.lane));}));
document.addEventListener('keydown',e=>{if($('method-dialog').open||['INPUT','SELECT'].includes(document.activeElement.tagName))return;const lane=['d','f','j','k'].indexOf(e.key.toLowerCase());if(lane>=0&&!e.repeat){e.preventDefault();press(lane);}if(e.code==='Space'&&!e.repeat&&!recording&&document.activeElement.tagName!=='BUTTON'){e.preventDefault();$('start').click();}});
...
$('sound').onclick=async()=>{
  ...
  if (sound) {
    ...
    if (shown?.mode === 'running') {
      songAudio.currentTime = Math.max(0, shown.game.time);
      songAudio.play().catch(e => console.warn('Audio play:', e));
    } else {
      // Audio unlock & quick audible confirmation feedback
      songAudio.currentTime = 0.8;
      songAudio.play().then(() => {
        setTimeout(() => {
          if (shown?.mode !== 'running' && songAudio) {
            songAudio.pause();
            songAudio.currentTime = Math.max(0, shown?.game?.time || 0);
          }
        }, 750);
      }).catch(e => console.warn('Audio gesture feedback:', e));
    }
  } else {
    if (songAudio && !songAudio.paused) songAudio.pause();
  }
...
$('replay-play').onclick=()=>{if(replayIndex===recording.frames.length-1){replayIndex=0;display(recording.frames[0]);$('scrub').value=0;}playback=!playback;replayOffset=recording.frames[replayIndex].game.time-recording.frames[0].game.time;replayStart=performance.now();$('replay-play').textContent=playback?'Pausar':'Continuar';};
$('scrub').oninput=e=>{replayIndex=Number(e.target.value);replayOffset=recording.frames[replayIndex].game.time-recording.frames[0].game.time;replayStart=performance.now();history=[];display(recording.frames[replayIndex]);};
...
function drawBrain(){
  ...
  // Smooth decay of dopamine reward pulse
  dopamineRewardPulse=Math.max(0,dopamineRewardPulse-0.022);
  const legDopa=$('legend-dopamine');
  if(legDopa)legDopa.classList.toggle('reward-glow',dopamineRewardPulse>0.15);

  // Smooth decay of acoustic cueca pulse
  const curAcoustic=shown?.neural?.acoustic_energy||0.0;
  acousticPulse=Math.max(curAcoustic,acousticPulse-0.028);
  const legAud=$('legend-auditory');
  if(legAud)legAud.classList.toggle('auditory-glow',acousticPulse>0.20);
  ...
  if(edges.length>0){
    ...
    for(let e=0;e<edges.length;e++){
      ...
      brainCtx.beginPath();
      brainCtx.strokeStyle=cPre?'rgba(197, 164, 238, 0.72)':'rgba(117, 208, 177, 0.62)';
  ...
  // 3. Draw Somas (back-to-front depth sorted)
  for(const p of points){
    const active=counts[p.i]>0;
    const role=p.role;

    if(role==='dopamine'){
      // Dopamina: Ámbar cálido (#f59e0b) en reposo; destello dorado (#ffd54f) en premio
      const isGlowing=active||dopamineRewardPulse>0.04;
      const glow=dopamineRewardPulse+(active?0.5:0);

      brainCtx.save();
      if(isGlowing){
        brainCtx.shadowColor='#ffca28';
        brainCtx.shadowBlur=(2.5+glow*4.5)*p.p;
        brainCtx.fillStyle='#fff3b0';
      }else{
        brainCtx.fillStyle='#f59e0b';
      }

      const baseRadius=(active?2.3:1.35)*(1+glow*0.18)*p.p;
      brainCtx.globalAlpha=Math.min(1.0,(active?0.95:Math.max(0.35,0.75-p.z*0.15))+glow*0.25);
      brainCtx.beginPath();
      brainCtx.arc(p.x,p.y,Math.max(0.4,baseRadius),0,Math.PI*2);
      brainCtx.fill();
      brainCtx.restore();

    }else if(role==='auditory'){
      // Audición (Órgano de Johnston · AMMC/WED): Verde esmeralda (#10b981 idle / #34d399 active)
      const isAudActive=active||acousticPulse>0.10;
      const audGlow=acousticPulse+(active?0.5:0);

      brainCtx.save();
      if(isAudActive){
        brainCtx.shadowColor='#10b981';
        brainCtx.shadowBlur=(3.0+audGlow*5.0)*p.p;
        brainCtx.fillStyle='#6ee7b7';
        brainCtx.globalAlpha=0.98;
      }else{
        brainCtx.fillStyle='#10b981';
        brainCtx.globalAlpha=Math.max(0.28,0.70-p.z*0.18);
      }
      const radius=(isAudActive?2.8:1.25)*(1+audGlow*0.15)*p.p;
      brainCtx.beginPath();
      brainCtx.arc(p.x,p.y,Math.max(0.5,radius),0,Math.PI*2);
      brainCtx.fill();
      brainCtx.restore();

    }else if(role==='stimulus'){
      // Estímulos sensoriales/visuales: Cyan eléctrico (#38bdf8 idle / #00f5d4 active)
      brainCtx.save();
      if(active){
        brainCtx.shadowColor='#00f5d4';
        brainCtx.shadowBlur=10*p.p;
        brainCtx.fillStyle='#00f5d4';
        brainCtx.globalAlpha=0.98;
      }else{
        brainCtx.fillStyle='#38bdf8';
        brainCtx.globalAlpha=Math.max(0.25,0.65-p.z*0.20);
      }
      const radius=(active?2.9:1.25)*p.p;
      brainCtx.beginPath();
      brainCtx.arc(p.x,p.y,Math.max(0.5,radius),0,Math.PI*2);
      brainCtx.fill();
      brainCtx.restore();
      ...
      // Instrucciones a las patas: Coral/Magenta (#f43f5e idle / #ff007f active)
      brainCtx.save();
      if(active){
        brainCtx.shadowColor='#ff007f';
        brainCtx.shadowBlur=10*p.p;
        brainCtx.fillStyle='#ff007f';
        brainCtx.globalAlpha=0.98;
      }else{
        brainCtx.fillStyle='#f43f5e';
        brainCtx.globalAlpha=Math.max(0.25,0.68-p.z*0.20);
      }
      const radius=(active?3.0:1.3)*p.p;
      brainCtx.beginPath();
      brainCtx.arc(p.x,p.y,Math.max(0.5,radius),0,Math.PI*2);
      brainCtx.fill();
      brainCtx.restore();
      ...
      // Red intrínseca (interneuronas centrales): Lavanda / violeta suave (#715e85 idle / #d8b4fe active)
      brainCtx.fillStyle=active?'#d8b4fe':'#715e85';
      brainCtx.globalAlpha=active?0.95:Math.max(0.18,0.55-p.z*0.20);
      const radius=(active?2.4:1.0)*p.p;
      brainCtx.beginPath();
      brainCtx.arc(p.x,p.y,Math.max(0.4,radius),0,Math.PI*2);
      brainCtx.fill();
    }
  }
}
...
function render(now){if(recording&&playback){const target=recording.frames[0].game.time+replayOffset+(now-replayStart)/1000;let next=replayIndex;while(next<recording.frames.length-1&&recording.frames[next+1].game.time<=target)next++;if(next!==replayIndex){replayIndex=next;display(recording.frames[next]);$('scrub').value=next;}if(next===recording.frames.length-1){playback=false;$('replay-play').textContent='Repetir';}}if(frameImage)gameCtx.drawImage(frameImage,0,0,320,400);stage?.render(now);drawBrain();drawSpark();requestAnimationFrame(render);}
...
function renderTrainingDashboard(data, compData) {
  ...
  if ($('kpi-acc-gain')) $('kpi-acc-gain').textContent = sum.accuracy_gain != null ? `+${sum.accuracy_gain.toFixed(1)}%` : '0%';
  ...
  if (tbody) {
    tbody.innerHTML = '';
    const maxAcc = Math.max(...curve.map(c => c.mean_accuracy || 0));

    curve.forEach((pt, idx) => {
      const tr = document.createElement('tr');
      const isBest = (pt.mean_accuracy || 0) === maxAcc && maxAcc > 0;
      ...
      tr.innerHTML = `
        ...
        <td><strong>${(pt.mean_accuracy || 0).toFixed(1)}%</strong></td>
        <td>${(pt.mean_hits || 0).toFixed(1)}</td>
        <td>${(pt.mean_wrong || 0).toFixed(1)}</td>
        <td>${format(Math.round(pt.mean_score || 0))}</td>
  ...
  drawTrainingChart(curve, compData?.curve);
}

function drawTrainingChart(curve, compareCurve) {
